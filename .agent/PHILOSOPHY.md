...
The flat `whoop/` package layout and Functional Options pattern (`whoop.NewClient(whoop.WithToken("..."))`) provide an ergonomic and instantly recognizable developer experience. Sensible defaults (3 retries, 1s base backoff, 60s max backoff, 30s HTTP timeout) mean the zero-config path just works.
